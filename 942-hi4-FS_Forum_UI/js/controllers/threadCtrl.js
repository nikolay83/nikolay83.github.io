/* thread module */
var ngcControllers = angular.module('threadModule', [])

// thread controller
.controller('threadCtrl', function($scope, $rootScope, $http, $routeParams) {

    $scope.thread = {};
    if ($routeParams.thread) {
        $scope.thread.title = $routeParams.thread.replace(':', '');
    }
    ...
    $rootScope.crumble.trail = [{
        ...
        label: 'Threads of ' + $scope.thread.title,
        path: "#/Thread:" + $scope.thread.title
    }];
...
/** !threadDeatisCtrl */
.controller('threadDeatisCtrl', function($scope, $rootScope, $http, $routeParams) {
    ...
    //get JSON data
    $scope.forumData = {};
    $http.get("data/threadDetails.json")
        .then(function(res) {
            $scope.data = res.data;
            $scope.currentThreadPost = $scope.data.threadDeatils[2];
            $scope.newThread.title = $scope.currentThreadPost.title;
        });
    ...
    $scope.currentThreadPost = {};


    $scope.replyPost = function(post){
        $scope.replying = true;
        $scope.currentThreadPost  = post;
    };
    $scope.cancelReplyPost = function(){
        $scope.replying = false;
    };
});
