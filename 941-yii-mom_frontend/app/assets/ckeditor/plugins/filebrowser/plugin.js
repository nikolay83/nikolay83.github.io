...
( function() {
	...
	var file = dialog.getContentElement(this[ 'for' ][ 0 ], this[ 'for' ][ 1 ]).getInputElement().$.files[0];
	if (file) {
	    if (CKEDITOR.fileTools.isTypeSupported(file, /image\/(png|jpeg|gif)/)) {
		var loader = editor.uploadRepository.create(file);
		loader.on('update', function() {
		    editor.fire('fileUploadProgress', loader);
		});
		loader.on('error', function() {
		    alert('Error!');
		});
		loader.loadAndUpload(!dialog.getContentElement(this[ 'for' ][ 0 ], this[ 'for' ][ 1 ]).getAction());
	    }
	}
